...
def train(
    ...
    if config.wandb.use:
        ...
        if sweep:
            wandb.init(
                name="train_model_{}_method_{}_mask_{}".format(
                config.model.model_class, config.method.model_kwargs.method_name, config.encoder.masker.mode)
            )
            config.encoder.masker.ratio = wandb.config.mask_ratio
            config.optimizer.wd = wandb.config.weight_decay
        else:
            wandb.init(
            ...
            config.encoder.masker.ratio = hyper_params["mask_ratio"]
            config.optimizer.wd = hyper_params["weight_decay"]
