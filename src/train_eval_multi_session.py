...
ap.add_argument("--test_eid", type=str)
...
if config.wandb.use:
    import wandb
    wandb.init(project=config.wandb.project, entity=config.wandb.entity, config=config, name="train_multi_session_model_{}_method_{}_mask_{}_stitch_{}".format(config.model.model_class, config.method.model_kwargs.method_name,config.model.encoder.masker.mode, config.model.encoder.stitching))
