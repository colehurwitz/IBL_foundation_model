...
def load_model_data_local(meta_data, **kwargs):
    ...
    config['model']['encoder']['embedder']['n_channels_per_region'] = kwargs['region_channel_num']
...
def co_smoothing_eval(
    ...
    assert n == 1, 'only support n=1 now' #number of neurons to mask out in per_neuron mode

    for batch in test_dataloader:
        eid_eval = batch['eid'].unique().item()
        tot_num_trials = len(batch['spikes_data'])
        print('perform evaluation on session: ', eid_eval)
        break
    
    record_info_list = kwargs['record_info_list']
    eids = kwargs['eids']
    ...
    uuids_list = np.arange(tot_num_neurons)  # N, 
    region_list = batch['neuron_regions'][0] # N, 
    ...
    if is_aligned:
        ...
        var_value2label = {'choice': {(0.0,): "right",
                                       (1.0,): "left"}}
        var_tasklist = ['choice']
        #var_behlist = ['jaw', 'nose', 'tongue']
        var_behlist = []

    if mode == 'per_neuron':
        
        bps_result_list, r2_result_list, fr_list = [float('nan')] * tot_num_neurons, [np.array([np.nan]*4)] * N, [float('nan')] * N
        ...
        for n_i in tqdm(range(0, tot_num_neurons+n_jobs, n_jobs)):    
            ...
            with torch.no_grad():
                for batch in test_dataloader:
                    ...
                    gt_fr_data = batch['fr'][:,:,neuron_id_record].clone()
                    ...
                    outputs = model(
                        ...
                        neuron_regions=torch.cat(neuron_regions_lst,0),
                    ...
                    #behavior_ind = [0,3,6]
                    behavior_ind = [0]
                    behavior_set = targets_lst[0][:,:T, behavior_ind].detach().cpu().numpy()
                    #behavior_set = np.concatenate([np.ones((behavior_set.shape[0], T, 1)), behavior_set], axis=-1)
                    break # only one batch used for test_data_loader
            ...
            gt_fr = torch.cat(gt_fr_lst, 0).detach().cpu().numpy()
            ...
            for i in range(n_jobs):
                if n_i+i < tot_num_neurons:
                    ...
                    gt_fr_held_out = gt_fr[i*tot_num_trials:(i+1)*tot_num_trials, :, [n_i+i]]
                    ...
                    if is_aligned:
                        ...
                        _r2_psth, _r2_trial, _r2_psth_fr, _r2_single_trial_fr = viz_single_cell(X, gt_held_out.squeeze(), pred_held_out.squeeze(), gt_fr_held_out.squeeze(),
                                                              var_name2idx, var_tasklist, var_value2label, var_behlist,
                                                              subtract_psth=kwargs['subtract'],
                                                              aligned_tbins=kwargs['onset_alignment'],
                                                              neuron_idx=uuids_list[n_i+i],
                                                              neuron_region=region_list[n_i+i],
                                                              method=method_name, save_path=kwargs['save_path'], save_plot=True)
                        r2_result_list[n_i+i] = np.array([_r2_psth, _r2_trial, _r2_psth_fr, _r2_single_trial_fr])
                        fr_list[n_i+i] = np.mean(gt_held_out.squeeze())*100
                    else:
        ...
        for hd_idx in held_out_list:
            ...
            with torch.no_grad():
                for batch in test_dataloader:
                    ...
                    #behavior_ind = [0,3,6]
                    behavior_ind = [0]
                    behavior_set = batch['target'][:,:T, behavior_ind].detach().cpu().numpy()
                    #behavior_set = np.concatenate([np.ones((behavior_set.shape[0], T, 1)), behavior_set], axis=-1)
            ...
            gt_fr_plot = gt_spikes[:, target_time_idxs]
            ...
            for i in tqdm(range(idxs.shape[0]), desc='R2'):
                if is_aligned:
                    ...
                    _r2_psth, _r2_trial = viz_single_cell(X, ys[:, :, idxs[i]], y_preds[:, :, idxs[i]], gt_fr_plot[:, :, idxs[i]],
                                                          var_name2idx, var_tasklist, var_value2label, var_behlist,
                                                          subtract_psth=kwargs['subtract'],
                                                          aligned_tbins=[],
                                                          neuron_idx=uuids_list[idxs[i]],
        ...
        bps_result_list, r2_result_list, fr_list = [float('nan')] * tot_num_neurons, [np.array([np.nan, np.nan])] * N, np.nan*np.ones((N,))
        for region in tqdm(target_regions, desc='region'):
            ...
            gt_spikes = gt_spike_data.detach().cpu().numpy()
            pred_spikes = outputs.preds.detach().cpu().numpy()
            gt_fr = gt_fr_data.detach().cpu().numpy()
            ...
            gt_held_out = gt_spikes[:, target_time_idxs][:,:,target_neuron_idxs]
            pred_held_out = pred_spikes[:, target_time_idxs][:,:,target_neuron_idxs]
            gt_fr_held_out = gt_fr[:, target_time_idxs][:,:,target_neuron_idxs]

            fr_list[target_neuron_idxs] = np.mean(gt_held_out.squeeze(),(0,1))*100
            ...
            ys = gt_spikes[:, target_time_idxs]
            y_preds = pred_spikes[:, target_time_idxs]
            gt_fr_plot = gt_fr[:, target_time_idxs]
            ...
            for i in range(idxs.shape[0]):
                if is_aligned:
                    X = behavior_set[:, target_time_idxs, :]  # [#trials, #timesteps, #variables]
                    _r2_psth, _r2_trial,_r2_psth_fr, _r2_single_trial_fr = viz_single_cell(X, ys[:, :, idxs[i]], y_preds[:, :, idxs[i]], gt_fr_plot[:, :, idxs[i]],
                                                          var_name2idx, var_tasklist, var_value2label, var_behlist,
                                                          subtract_psth=kwargs['subtract'],
                                                          aligned_tbins=[],
                                                          neuron_idx=uuids_list[idxs[i]],
                                                          neuron_region=region_list[idxs[i]],
                                                          method=method_name, save_path=kwargs['save_path'], save_plot=True);
                    r2_result_list[idxs[i]] = np.array([_r2_psth, _r2_trial, _r2_psth_fr, _r2_single_trial_fr])
                else:
                    r2 = viz_single_cell_unaligned(
                        ys[:, :, idxs[i]], y_preds[:, :, idxs[i]], 
                        neuron_idx=uuids_list[idxs[i]],
            ...
            for hd_idx in held_out_list:
                ...
                with torch.no_grad():
                    for batch in test_dataloader:
                        ...
                        behavior_ind = [0,3,6]
                        behavior_set = torch.cat(targets_lst, 0)[:,:T, behavior_ind].detach().cpu().numpy()
                ...
                for i in range(len(heldout_idxs)):
                    ...
                    if is_aligned:
                        ...
                        _r2_psth, _r2_trial = viz_single_cell(X, gt_held_out.squeeze(), pred_held_out.squeeze(),
                                                              ...
                                                              subtract_psth=kwargs['subtract'],
                                                              aligned_tbins=[],
                                                              neuron_idx=uuids_list[heldout_idxs[i]],
    ...
    fr_all = np.array(fr_list)

    #save r2_all, fr_all
    np.save(os.path.join(kwargs['save_path'], f'r2.npy'), r2_all)
    np.save(os.path.join(kwargs['save_path'], f'fr.npy'), fr_all)
        ...
        for ci, var in enumerate(var_tasklist):
            ...
            for _i, _x in enumerate(psth_xy.keys()):
                ...
                ax.plot(psth,
                        ...
                        linewidth=1, alpha=0.3, label=f"{var_value2label[var][tuple(_x)]}")
                ax.plot(psth_pred,
                        color=plt.get_cmap('tab10')(_i),
                        linestyle='--')
...
def plot_single_trial_activity(X, y, y_pred, fr_gt,
                               var_name2idx,
                               var_behlist,
                               var_tasklist, subtract_psth="task",
                               aligned_tbins=[],
                               cmap='bwr', vmax_perc=99, vmin_perc=1,
                               axes=None):
    if axes is None:
        ncols = 1;
    ...
    if subtract_psth is None:
        ...
        psth_xy = compute_all_psth(X, y, idxs_psth)
        psth_pred_xy = compute_all_psth(X, y_pred, idxs_psth)
        psth_xfr = compute_all_psth(X, fr_gt, idxs_psth)
        y_psth = np.asarray(
        ...
        frpsth = np.asarray(
            [psth_xfr[tuple(x)] for x in X[:, 0, idxs_psth]])

        y_variability = y - y_psth  # (K, T)
        y_pred_variability = y_pred - y_predpsth  # (K, T)
        fr_variability = fr_gt - frpsth
    ...
    y_residual = (y_pred - fr_gt)  # (K, T), residuals of prediction
    idxs_behavior = np.concatenate(([var_name2idx[var] for var in var_behlist])) if len(var_behlist) > 0 else []
    X_behs = X[:, :, idxs_behavior] #there is NO behavior traces in surrogate data

    t_sort = np.argsort(X[:,0,0]) # sort by choice

    for ri, (toshow, label, ax) in enumerate(zip([y, y_pred, fr_gt,
                                                  y_variability, y_pred_variability, fr_variability,
                                                   X_behs, y_residual],
                                                 ['obs. act.', 'pred. act.', 'fr_gt',
                                                  f"obs. act. \n (subtract_psth={subtract_psth})",
                                                  f"pred. act. \n (subtract_psth={subtract_psth})",
                                                  f"fr_gt \n (subtract_psth={subtract_psth})",
                                                  var_behlist,
                                                  "residual act."],
                                                 [axes[0], axes[1], axes[2], axes[3], axes[4], axes[5], axes[6:-1], axes[-1]])):
        if ri <= 5:
            # plot obs./ predicted activity
            vmax = np.percentile(y, vmax_perc)
            vmin = np.percentile(y, vmin_perc)

            if ri>2:
                vmax = np.percentile(fr_variability, vmax_perc)
                vmin = np.percentile(fr_variability, vmin_perc)
...
def viz_single_cell(X, y, y_pred, fr_gt, 
                    var_name2idx, var_tasklist, var_value2label, var_behlist,
                    subtract_psth="task", aligned_tbins=[], neuron_idx='', neuron_region='', method='',
    ...
    if save_plot:
        ...
        axes_single = [plt.subplot(nrows, 1, k) for k in range(2, 9 + len(var_behlist))]
    else:
        axes_psth = None
        axes_single = None
    ...
    #calculate R2 for fr_gt
    idxs_psth = np.concatenate([var_name2idx[var] for var in var_tasklist])
    psth_xy_fr = compute_all_psth(X, fr_gt, idxs_psth)
    psth_pred_xy = compute_all_psth(X, y_pred, idxs_psth)
    r2_psth_fr = compute_R2_psth(psth_xy_fr, psth_pred_xy, clip=False)
    r2_single_trial_fr = compute_R2_main(fr_gt.reshape(-1, 1), y_pred.reshape(-1, 1), clip=False)[0]
    


    ### plot the psth-subtracted activity
    if save_plot:
        plot_single_trial_activity(X, y, y_pred,fr_gt,
                                   var_name2idx,
                                   var_behlist,
