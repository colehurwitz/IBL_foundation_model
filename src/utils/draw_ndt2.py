...
TASK2NAME = {'co_smooth': 'Co-Smooth', 'forward_pred':'Forward Prediction', 'intra_region':'Intra-Region', 'inter_region':'Inter-Region','choice_decoding':'Choice', 'continuous_decoding':'Whisker\n Motion Energy'}

if model == "NDT2":
    model_name = 'STPatch'
    mask_methods = ['mask_all_prompt', 'mask_random_token']
else:
    model_name = 'NDT1'
...
for eid in eids:
            ...
            if eval == "choice_decoding":                
                try:
                    ...
                    if model == "NDT2":
                        ...
                        acc = acc_list['choice_intra-region']
                ...
                try:
                    ...
                    if model == "NDT2":
                        ...
                        r2 = r2_list['whisker-motion-energy_intra-region']
    ...
    for i, ax in enumerate(axes):
        if model == "NDT2":
            ax.set_yticks(np.arange(N), labels=['all mask', 'random token mask'])
        else:
...
for i, task in enumerate(eval_methods):
    ...
    for eid in eids:
        if model == 'NDT1':
            ...
            x = metrics_dict[eid]['mask_random_token'][task]['bps']
        y = metrics_dict[eid]['mask_all_prompt'][task]['bps']
        ax.scatter(x, y,s=60,c='dodgerblue')
        x_list.append(x)
        y_list.append(y)
...
for i, task in enumerate(finetune_methods):
    ...
    for eid in eids:
        if model == 'NDT1':
            ...
            x = metrics_dict[eid]['mask_random_token'][task]['metric']
        y = metrics_dict[eid]['mask_all_prompt'][task]['metric']
        ax.scatter(x, y,s=60,c='r')
        x_list.append(x)
        y_list.append(y)
    ax.plot(linear_line,linear_line, c='k', ls='--', lw=2)
