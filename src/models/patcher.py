...
class Patcher(nn.Module):
    ...
    def forward(
        ...
        # Group spike masks into patches
        target_masks = torch.ones((B, self.n_time_patches, self.n_space_patches, self.n_channels))     
        for t in range(self.n_time_patches):
            for s in range(self.n_space_patches):
                target_masks[:,t,s] = spikes_masks[:,t*self.max_time_F:(t+1)*self.max_time_F, \
                s*self.max_space_F:(s+1)*self.max_space_F].flatten(1)
