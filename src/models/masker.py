...
class Masker(nn.Module):
    ...
    def forward(
        ...
        #xj
        neuron_regions = neuron_regions.cpu().numpy()

        if 'all' in self.mask_regions:
            self.unique_regions = list(np.unique(neuron_regions))
        ...
        if self.mode in ["temporal", "random_token", "causal"]:
            ...
            current_mask_regions = random.sample(self.unique_regions, self.n_mask_regions)
            #print('now mask regions:', current_mask_regions, '# of neurons: ', len(neuron_regions[0,:]))
            mask_probs = torch.zeros(spikes.shape[0],spikes.shape[2])
            for region in current_mask_regions:
