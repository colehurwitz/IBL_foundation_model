...
class NeuralEmbeddingLayer(nn.Module):
    ...
    def __init__(self, hidden_size, n_channels, config: DictConfig):
        ...
        if self.use_session:
            self.embed_session = nn.Embedding(173, hidden_size) 
    ...
    def forward(
        ...
        if self.tokenize_binary_mask:
            # this doesn't work, because the model didn't learn to match the binary_mask to spikes
        ...
        if self.use_session:
            session_idx = torch.tensor(eid, dtype=torch.int64, device=spikes.device)
    ...
    # because self.stack = False, this function can be ignored
...
class NeuralEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Build stitcher
        if config.stitching:
            self.stitcher = NeuralStitcher(kwargs['num_neurons'], kwargs['eids'], config.embedder.n_channels)
    ...
    def forward(
        ...
        if hasattr(self, 'stitcher'):
            #spikes = self.stitcher(spikes, str(eid.item()), neuron_regions)
            spikes = self.stitcher(spikes, str(eid.item()))
        # Embed neural data
        ...
        for idx, layer in enumerate(self.layers):
            x = layer(x, timestamp=spikes_timestamp)
        x = self.out_norm(x)
...
class NeuralStitcher(nn.Module):
    ...
    def __init__(self, 
        ...
        stitcher_dict = {}
        for num_neuron, session_ind in zip(num_neurons, session_list):
            stitcher_dict[str(session_ind)] = nn.Linear(num_neuron, n_channels)
    ...
    def __init__(self,
        ...
        for area_ind_list, session_ind in zip(area_ind_list_list, session_list):
            ...
            for area_ind in area_ind_unique:
                ...
                stitch_decoder_dict[str(session_ind)][str(area_ind)] = nn.Linear(n_channels_per_region, len(neuron_indices))
        self.stitch_decoder_dict = nn.ModuleDict({k: nn.ModuleDict(v) for k, v in stitch_decoder_dict.items()})
        
        n_tot_areas = len(areaoi_ind)
        self.areaoi_ind = torch.tensor(areaoi_ind)
        self.Linear = nn.Linear(n_channels, n_channels_per_region*n_tot_areas)
        self.size_mid = n_channels_per_region*n_tot_areas
        self.n_channels_per_region = n_channels_per_region

    def forward(self, x, block_idx, neuron_regions):
        
        stitch_decoder = self.stitch_decoder_dict[block_idx]
        area_ind_unique = torch.unique(neuron_regions)
        x1 = self.Linear(x)

        x2_list = []
        areaoi_ind = self.areaoi_ind.to(x.device)

        for area_ind in area_ind_unique:
            area_index = torch.where(areaoi_ind == area_ind)[0]
            x2 = stitch_decoder[str(area_ind.item())](x1[:,:, area_index*self.n_channels_per_region: (area_index+1)*self.n_channels_per_region])
            x2_list.append(x2)

        return torch.cat(x2_list, dim=2)

# Encoder for time binned neural data
class NDT1_with_region_stitcher(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if config.encoder.stitching:
            ...
            self.stitch_decoder = StitchDecoder(kwargs['eids'], self.hidden_size, config.encoder.embedder.n_channels_per_region, kwargs['area_ind_list_list'], kwargs['areaoi_ind'])
        else:
