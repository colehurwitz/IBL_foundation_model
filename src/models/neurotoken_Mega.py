...
#         # Encoder block
#         self.ln1 = ScaleNorm(config.MEGA_hidden_size ** 0.5) if config.use_scalenorm else nn.LayerNorm(config.MEGA_hidden_size) 
#         self.attn = NeuralAttention(idx, config.MEGA_hidden_size, config.n_heads, config.attention_bias, config.dropout, config.use_rope, config.rope_theta, max_F)
#         self.ln2 = ScaleNorm(config.MEGA_hidden_size ** 0.5) if config.use_scalenorm else nn.LayerNorm(config.MEGA_hidden_size) 
#         self.mlp = NeuralMLP(config.MEGA_hidden_size, config.inter_size, config.act, config.mlp_bias, config.dropout)
...
#         self.out_norm = ScaleNorm(self.MEGA_hidden_size ** 0.5) if config.transformer.use_scalenorm else nn.LayerNorm(self.MEGA_hidden_size) 
       
#         # Out projection
#         self.out_proj = NeuralFactorsProjection(self.MEGA_hidden_size, config.factors)
...
#                 self.region_embeddings = nn.Sequential(
#                     nn.Embedding(
#                         self.embedder.n_space_patches * self.regionlookup.max_region_indx, self.MEGA_hidden_size),
#                     nn.LayerNorm(self.MEGA_hidden_size),
#                 )

#             self.embed_space_pos = nn.Sequential(
#                 nn.Embedding(
#                     self.embedder.n_space_patches, self.MEGA_hidden_size
#                 ),
#                 nn.LayerNorm(self.MEGA_hidden_size),
#             )
...
#             megaX = layer(megaX, attn_mask=attn_mask) 
#         megaX = self.out_norm(megaX)

#         #NOTE: _T value may be wrong here 
#         return self.out_proj(megaX), targets_mask, _T     #from ndt1, gives output of out_proj_size, might want to do encoder.hidden_size instead  
...
#             decoder_layers.append(nn.Linear(self.encoder.out_proj.out_size, self.encoder.out_proj.out_size // 3))
#             decoder_layers.append(nn.ReLU())
#             decoder_layers.append(nn.Linear(self.encoder.out_proj.out_size // 3, self.encoder.out_proj.out_size // 3))
#             decoder_layers.append(nn.ReLU())
#             decoder_layers.append(nn.Linear(self.encoder.out_proj.out_size // 3, self.encoder.n_timesteps))
