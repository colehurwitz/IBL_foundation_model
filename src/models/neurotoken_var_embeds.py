...
class NeuralEncoder(nn.Module):
    ...
    def forward(
        ...
        # #NEW ADD - apply time mask manually before making megatoken
        # x = x * _time_attn_mask.unsqueeze(-1).expand(B,T,N_embed).to(x.dtype)  #NEW ADD

        #reshape x to (B,num_neurons,neuron_combined_hidden_size) -- NEW ADD
        # x = x.reshape(B, self.embedder.n_time_patches, self.embedder.n_space_patches, self.hidden_size)
        # neuron_combined_hidden_size = self.hidden_size * self.embedder.n_time_patches
        # x = x.permute(0,2,1,3).reshape(B, self.embedder.n_space_patches, neuron_combined_hidden_size)
        #get new T
        _, T, _ = x.size()

        # #make megatoken to go from (B,num_tokens,hidden_size) to (B,num_neurons,MEGAhidden_size)
        # megaX = self.megatokenizer(x)  #NEW ADD

        # # Embed position
        # if self.pos:
            # simple_spacestamps = torch.arange(self.embedder.n_space_patches).to(torch.int64)[None,:].to(spikes.device)
            # megaX += self.embed_space_pos(simple_spacestamps)  #added
            # x += self.embed_time_pos(timestamps)    
            # x += self.embed_space_pos(spacestamps) 

        # if self.embed_region:      #TODO: need to fix dims for the input/output                
        #     region_embeds = self.region_embeddings(regionstamps)   #added
        #     megaX += region_embeds      #added
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.method == "sl":     #TODO: figure out how this should be changed
            ...
            decoder_layers.append(nn.Linear(self.encoder.out_proj.out_size, self.encoder.out_proj.out_size // 2))
            decoder_layers.append(nn.ReLU())
            decoder_layers.append(nn.Linear(self.encoder.out_proj.out_size // 2, self.encoder.out_proj.out_size // 3))
            decoder_layers.append(nn.ReLU())
...
class TCN(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=3):
        ...
        for i in range(len(num_channels)):
            dilation_size = 2 ** i  # exponential growth of dilation
...
class FullyConnectedReduction(nn.Module):
    def __init__(self, num_time_points, num_channels, hidden_size):
        super(FullyConnectedReduction, self).__init__()
        # Fully connected layer to map the flattened output of TCN to the hidden size
        # self.fc = nn.Linear(num_time_points * num_channels, hidden_size)
        self.fc = nn.Linear(64 * num_channels, hidden_size)     #TODO: need to unhardcode this

    def forward(self, x):
        # Flatten the TCN output along the time and channel dimensions
        x = x.view(x.size(0), -1)  # Flatten (batch_size * num_tokens, num_channels[-1] * num_time_points)
        return self.fc(x)  # Shape: (batch_size * num_tokens, hidden_size)

class NeuralDataTCN(nn.Module):
    def __init__(self, num_time_points, hidden_size, num_channels):
        super(NeuralDataTCN, self).__init__()
        # TCN expects 1 input channel (time points are treated as sequences)
        self.tcn = TCN(1, num_channels)
        # Fully connected layer to reduce TCN output to hidden_size
        self.fc_reduction = FullyConnectedReduction(num_time_points, num_channels[-1], hidden_size)

    def forward(self, x):
        batch_size, num_tokens, num_time_points = x.shape
        
        # Reshape to (batch_size * num_tokens, 1, num_time_points) so each neuron is processed independently
        x = x.view(batch_size * num_tokens, 1, num_time_points)
        
        # Pass each neuron's time series through the TCN
        x = self.tcn(x)  # Shape becomes (batch_size * num_tokens, num_channels[-1], num_time_points)
        
        # Flatten and reduce using fully connected layer
        x = self.fc_reduction(x)  # Shape becomes (batch_size * num_tokens, hidden_size)
        
        # Reshape back to (batch_size, num_tokens, hidden_size)
...
class LSTMTokenProcessor(nn.Module):
    def __init__(self, num_time_points, hidden_size, num_layers=1, bidirectional=False):
        super(LSTMTokenProcessor, self).__init__()
        # LSTM to process each neuron's time series independently
        self.lstm = nn.LSTM(input_size=num_time_points,
    ...
    def forward(self, x):
        ...
        # Initialize a list to store the LSTM outputs for each neuron
        all_neuron_outputs = torch.zeros(batch_size, num_tokens, self.lstm.hidden_size * (2 if self.lstm.bidirectional else 1), device=x.device)
        
        # Process each neuron's time series independently
        for token_idx in range(num_tokens):
            # Extract the time series for this neuron (Shape: (batch_size, num_time_points))
            neuron_time_series = x[:, token_idx, :].unsqueeze(1)  # Add a dimension to match LSTM input (Shape: (batch_size, 1, num_time_points))
            
            # Pass the time series through the LSTM
            lstm_out, _ = self.lstm(neuron_time_series)  # Shape: (batch_size, 1, hidden_size)
            
            # Take the last hidden state (final time point) from the LSTM (Shape: (batch_size, hidden_size))
            neuron_output = lstm_out[:, -1, :]
            
            # Store the output for this neuron
            all_neuron_outputs[:, token_idx, :] = neuron_output
        
        # Concatenate the neuron outputs along the tokens (neurons) dimension
        # (batch_size, num_tokens, hidden_size)
        # final_output = torch.stack(all_neuron_outputs, dim=1)
        
        return all_neuron_outputs