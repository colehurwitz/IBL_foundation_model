...
#%%
DEFAULT_CONFIG = "/mnt/smb/locker/miller-locker/users/jx2484/code/autoencoder/src/configs/ndt1_stitching.yaml"
...
class StitchDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        stitch_decoder_dict = {}
        for num_neuron, session_ind in zip(num_neurons, session_list):
            stitch_decoder_dict[str(session_ind)] = nn.Linear(n_channels, num_neuron)
...
class NDT1(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if config.encoder.stitching:
            ...
            self.stitch_decoder = StitchDecoder(kwargs['num_neurons'], kwargs['eids'], self.hidden_size)
        else:
    ...
    def forward(
        ...
        if hasattr(self, "stitching") and self.method == "ssl":
            outputs = self.stitch_decoder(x, str(eid.item()))
        else:
