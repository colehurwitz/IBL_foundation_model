...
class Trainer():
    ...
    def train(self):
        ...
        # After profiling, filter and print the least memory-intensive operations
        key_averages = prof.key_averages()
        
        # Sort by memory usage
        sorted_averages = sorted(key_averages, key=lambda x: x.cuda_memory_usage)

        # Print the least memory-intensive operations
        print("Least memory-intensive operations:")
        for avg in sorted_averages[-10:]:  # Adjust the number as needed
            print(f"{avg.key}: {avg.cuda_memory_usage} bytes")

        # Optionally, export profiling results to a file
        prof.export_chrome_trace(f"{self.log_dir}/train_trace_final.json")
    ...
    def train_epoch(self, epoch):
        ...
        for batch in tqdm(self.train_dataloader):
            ...
            # with record_function("forward_pass"):   #NEW
            outputs = self._forward_model_outputs(batch, masking_mode)
            loss = outputs.loss
            # with record_function("backward_pass"):  #NEW
            loss.backward()
            self.optimizer.step()
